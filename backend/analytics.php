...
function multilang_booking_calendar_show_analytics() {
	if (!current_user_can('manage_options')) {
		wp_die("Go away");
	}
	global $wpdb;
	global $abcUrl;
	wp_enqueue_script('mbc-analytics', $abcUrl.'backend/js/mbc-analytics.js', array('jquery'));
	...
	$output = '<h1>'.__('Analytics', 'multilang-booking-calendar').'</h1><div class="uk-grid uk-grid-small">';
	...
	$er = $wpdb->get_results('SELECT * FROM '.$wpdb->prefix.'mbc_calendars ORDER BY name ASC', ARRAY_A);
	$foreachcount = 0;
	foreach($er as $row) {
	...
	if($foreachcount == 0){ // Check if there are calendars
		$output .= "<p>".__('There are currently no calendars.', 'multilang-booking-calendar')."
					<a href=\"admin.php?page=multilang-booking-calendar-show-seasons-calendars\">
					".__('Please create a calendar first.', 'multilang-booking-calendar')."</a></p>";
	}else {
		// Getting Bookings and calculating availability
		$er = $wpdb->get_results('SELECT * FROM '.$wpdb->prefix.'mbc_bookings 
		...
		$output .= '
			<div class="uk-width-1-1">
				<div class="mbc-analytics-wrapper ">
					<span class="mbc-analytics-title"><h2>'.sprintf( __('Overall Occupation Rate for the next %d days', 'mbc-bookings'), $numberOfDays ).'</h2></span>
					<div class="mbc-analytics-content mbc-overallAvailability"></div>
				</div>	
			</div>';
		...
		$output .= '
			<div class="uk-width-1-1">
				<div class="mbc-analytics-wrapper ">
					<span class="mbc-analytics-title"><h2>'.sprintf( __('Occupation Rate of each calendar for the next %d days', 'mbc-bookings'), $numberOfDays ).'</h2></span>
					<div class="mbc-analytics-content mbc-availability"></div>
				</div>
			</div>';
		// End: Availability per Calendar line chart
		
		// Start: Number of requests line chart
		$requestRows = $wpdb->get_results('SELECT * FROM '.$wpdb->prefix.'mbc_requests 
		...
		$output .= '
			<div class="uk-width-1-1">
				<div class="mbc-analytics-wrapper ">
					<span class="mbc-analytics-title"><h2>'.sprintf( __('Number of requests for the next %d days', 'mbc-bookings'), $numberOfDays ).'</h2></span>
					<div class="mbc-analytics-content mbc-requests"></div>
				</div>
			</div>';
		...
		$requestRows = $wpdb->get_results('SELECT persons, count(persons) as personCount FROM '.$wpdb->prefix.'mbc_requests 
					WHERE date_from <= "'.$endDate.'" 
						AND date_to >= "'.$startDate.'" 
		...
		$output .= '
			<div class="uk-width-1-1 uk-width-medium-1-2">
				<div class="mbc-analytics-wrapper ">
					<span class="mbc-analytics-title"><h2>'.sprintf( __('Percentage per person for the next %d days', 'mbc-bookings'), $numberOfDays ).'</h2></span>
					<div class="mbc-analytics-title mbc-personpie"></div>
				</div>
			</div>';
		// End: Percentage per person pie chart
		
		// Start: Revenue by Calendar
		$mainEr = $wpdb->get_results('SELECT * FROM '.$wpdb->prefix.'mbc_calendars ORDER BY name ASC', ARRAY_A);
		$outputBar = '';
		$revenueLabels = array();
		$revenueData = array();
		foreach($mainEr as $mainRow) {
			...
			foreach($er as $row) { // Calculating average revenue (if Booking is starts earlier or ends later than configured time period)
				if($startDate <= $row["start"] AND $endDate >= $row["end"]) { // Booking is between start- and enddate
					...
					$revenue += round(($row["price"]/mbc_booking_dateDiffInDays(strtotime($row["end"]), strtotime($row["start"]))*mbc_booking_dateDiffInDays(strtotime($row["end"]), strtotime($startDate))),2);
				} elseif($startDate <= $row["start"] AND $endDate <= $row["end"]){ // Enddate of Booking is later
					$revenue += round(($row["price"]/mbc_booking_dateDiffInDays(strtotime($row["end"]), strtotime($row["start"]))*mbc_booking_dateDiffInDays(strtotime($endDate), strtotime($row["start"]))),2);
				} elseif($startDate >= $row["start"] AND $endDate <= $row["end"]){ // Enddate of Booking is later
					$revenue += round(($row["price"]/mbc_booking_dateDiffInDays(strtotime($row["end"]), strtotime($row["start"]))*mbc_booking_dateDiffInDays(strtotime($endDate), strtotime($startDate))),2);
				}
			}
		...
		$output .= '
			<div class="uk-width-1-1 uk-width-medium-1-2">
				<div class="mbc-analytics-wrapper ">
					<span class="mbc-analytics-title"><h2>'.sprintf( __('Average revenue per calendar for the next %d days', 'mbc-bookings'), $numberOfDays ).'</h2></span>
					<div class="mbc-analytics-title mbc-revenueBar"></div>
				</div>
			</div>';
		// End: Revenue by Calendar
		$output .= '</div>'; //Closing <div class="uk-grid">
		wp_localize_script( 'mbc-analytics', 'mbc_graphData', $localizedVars);
	}
	echo $output;
